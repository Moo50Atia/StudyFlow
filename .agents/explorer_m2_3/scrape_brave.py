...
def search_brave(query):
    print(f"Scraping Brave Search for: {query}")
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for r in soup.find_all('div', class_='snippet'):
                title_elem = r.find('span', class_='title')
                link_elem = r.find('a')
                snippet_elem = r.find('div', class_='snippet-description')
...
def main():
    ...
    with open('brave_results.txt', 'w', encoding='utf-8') as f:
    ...
    print("Brave search completed. Results saved to brave_results.txt")
