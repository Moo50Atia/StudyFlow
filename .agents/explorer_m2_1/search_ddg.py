...
async def run_search(browser, query, idx):
    ...
    # We can try both html.duckduckgo.com and lite.duckduckgo.com
    url = f"https://html.duckduckgo.com/html/?q={encoded_query}"
    print(f"Navigating to DDG: {url}")
    ...
    filename = f"search_ddg_{idx+1}.txt"
    ...
    html_filename = f"search_ddg_{idx+1}.html"
    html_filepath = os.path.join(os.path.dirname(__file__), html_filename)
    with open(html_filepath, "w", encoding="utf-8") as f:
        f.write(html)
        
    try:
        screenshot_path = os.path.join(os.path.dirname(__file__), f"screenshot_ddg_{idx+1}.png")
